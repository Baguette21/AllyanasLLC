...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const item of items) {
      if (!item.id || !item.name || typeof item.price !== 'number' || !item.category) {
    ...
    for (const category of categories) {
      if (!category.name || !category.id || typeof category.order !== 'number') {
...
// Update category
router.put('/categories/:id', (async (req: Request<CategoryParams, any, CategoryUpdateBody>, res: Response) => {
  try {
    ...
    if (!req.body || !req.body.category || !req.body.oldCategoryName) {
      res.status(400).json({ error: 'Invalid request body. Must include category and oldCategoryName.' });
      return;
    }
    ...
    data.items = data.items.map(item => {
      if (item.category === oldCategoryName) {
        console.log(`Updating item ${item.name} from category ${oldCategoryName} to ${updatedCategory.name}`);
        return { 
          ...item, 
