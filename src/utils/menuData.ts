...
export const menuData = `category,item_name,description,price
...
export function parseMenuData(): MenuItem[] {
  return menuData
    .split('\n')
    .slice(1) // Skip header row
    .map(row => {
      const [category, item_name, description, price] = row.split(',');
      return {
        category,
        item_name,
        description,
...
export function getCategories(): string[] {
  ...
  parseMenuData().forEach(item => categories.add(item.category));
